// Adapted from AnyAscii's https://raw.githubusercontent.com/anyascii/anyascii/0.3.1/impl/csharp/src/Transliteration.cs

using System;
using System.Buffers;
using System.Text;

namespace MusicBeePlugin.External.AnyAscii
{
    public static partial class Transliteration
    {
        public static string Transliterate(this string s)
        {
            ...
            ArrayBufferWriter writer = new ArrayBufferWriter(s.Length);
            foreach (Rune r in s.EnumerateRunes())
            {
                Transliterate(r, writer);
            }
            return Encoding.ASCII.GetString(writer.OutputAsSpan.ToArray());
        }
